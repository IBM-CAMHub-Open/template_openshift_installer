...
resource "aws_subnet" "ocp_public" {
  ...
  availability_zone       = "${format("${var.region}%s", element(var.availability_zones, count.index))}"
  map_public_ip_on_launch = false

  tags = "${merge(
    map("Name", "${format("${var.cluster_name}-public-%d", count.index + 1) }"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
)}"
}
...
resource "aws_subnet" "ocp_private" {
  ...
  tags = "${merge(
    map("Name", "${format("${var.cluster_name}-private-%d", count.index + 1)}"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
  )}"
}
...
resource "aws_route_table" "ocp_private_network" {
  ...
  tags = "${merge(
    map("Name", "${format("${var.cluster_name}-private-%1d", count.index + 1)}"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
  )}"
}
...
resource "aws_route_table" "ocp_public_network" {
  ...
  tags = "${merge(
    map("Name", "${format("${var.cluster_name}-public-%1d", count.index + 1)}"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
  )}"
}
...
resource "aws_security_group" "worker" {
  ...
  tags = "${merge(
    map("Name", "${var.cluster_name}-worker"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
  )}"
}
...
resource "aws_security_group" "bastion" {
  ...
  tags = "${merge(
    map("Name", "${var.cluster_name}-bastion"),
    map("kubernetes.io/cluster/${var.cluster_name} ", "owned")
  )}"
}
