...
module "deployVM_etcd" {
  ...
  vm_domain_name          = "${var.vm_domain_name}"
  vm_folder               = "${var.vm_folder}"
  vm_private_ssh_key      = "${length(var.vm_vm_os_private_ssh_key) == 0 ? "${tls_private_key.generate.private_key_pem}"     : "${base64decode(var.vm_vm_os_private_ssh_key)}"}"
  vm_public_ssh_key       = "${length(var.vm_os_public_ssh_key)  == 0 ? "${tls_private_key.generate.public_key_openssh}"  : "${var.vm_os_public_ssh_key}"}"
  network                 = "${var.network}"
  vm_ipv4_gateway         = "${var.vm_ipv4_gateway}"
...
module "deployVM_compute" {
  ...
  vm_domain_name          = "${var.vm_domain_name}"
  vm_folder               = "${var.vm_folder}"
  vm_private_ssh_key      = "${length(var.vm_vm_os_private_ssh_key) == 0 ? "${tls_private_key.generate.private_key_pem}"     : "${base64decode(var.vm_os_private_ssh_key)}"}"
  vm_public_ssh_key       = "${length(var.vm_os_public_ssh_key)  == 0 ? "${tls_private_key.generate.public_key_openssh}"  : "${var.vm_os_public_ssh_key}"}"
  network                 = "${var.network}"
  vm_ipv4_gateway         = "${var.vm_ipv4_gateway}"
...
module "host_prepare" {
  ...
  vm_hostname_list      = "${join(",",distinct(concat(keys(var.infra_hostname_ip), keys(var.master_node_hostname_ip), keys(var.etcd_node_hostname_ip), keys(var.lb_node_hostname_ip))))}"
  vm_domain_name           = "${var.vm_domain_name}"
...
module "config_inventory" {
  ...
  lb_node_hostname          = "${join(",",keys(var.lb_node_hostname_ip))}"
