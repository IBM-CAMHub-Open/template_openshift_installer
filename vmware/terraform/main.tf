...
module "config_inventory" {
  ...
  infra_node_ipv4_address   = "${join(",",values(var.infra_node_hostname_ip))}"
  master_node_ipv  = "${element(values(var.master_node_hostname_ip), 0)}"
  rh_user                   = "${var.rh_user}"
  rh_password               = "${var.rh_password}"
...
module "generate_kubeconfig" {
  ...
  openshift_user       = "${var.openshift_user}"
  openshift_port	     = "8443"
  openshift_server	   = "${var.enable_lb == "false" ? element(keys(var.master_node_hostname_ip),0) : element(keys(var.lb_node_hostname_ip),0)}.${var.vm_domain}"
